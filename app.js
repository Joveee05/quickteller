...
app.get("/billers_by_category", function (req, res) {
  curl.curl(
    "/api/v2/quickteller/categorys/" + req.query.categoryid + "/billers",
    res
  );
});
...
app.get("/biller_payment_items", function (req, res) {
  curl.curl(
    "/api/v2/quickteller/billers/" + req.query.billerid + "/paymentitems",
    res
  );
});
...
app.get("/validate_name", function (req, res) {
  curl.curl(
    "/api/v1/nameenquiry/banks/accounts/names?bankCode=" +
      bankCode +
      "&accountId=" +
      $accountId,
    res
